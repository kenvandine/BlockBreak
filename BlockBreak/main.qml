...
Game {
    ...
    Scene {
        ...
        gravity: Qt.point(
                     0,
                     0) //disable gravity by applying zero force in both the x and y directions
        ...
        PhysicsEntity {
            ...
            fixtures: [
                Edge {
                    vertices: [Qt.point(0, 0), Qt.point(0, height)]
                    categories: Fixture.Category4
                    collidesWith: Fixture.Category2
                },
                Edge {
                    vertices: [Qt.point(-500,
                                        height + 10), Qt.point(2000,
                                                               height + 10)]
                    categories: Fixture.Category4
                    collidesWith: Fixture.Category2
                    sensor: true
                ...
                Edge {
                    vertices: [Qt.point(width, height), Qt.point(width, 0)]
                    categories: Fixture.Category4
                    collidesWith: Fixture.Category2
                },
                Edge {
                    vertices: [Qt.point(width, 0), Qt.point(0, 0)]
                    categories: Fixture.Category4
                    collidesWith: Fixture.Category2
                }
            ]
        ...
        onPreSolve: {
            ...
            if (targetA.objectName === "block") {
                ...
                var timer = Qt.createQmlObject("import QtQuick 2.3; Timer {}",
                                               game)
                timer.interval = 10
                timer.repeat = false
                ...
                var timerB = Qt.createQmlObject("import QtQuick 2.3; Timer {}",
                                                game)
                timerB.interval = 50
                timerB.repeat = false
            ...
            if (gameScene.score == gameScene.nextScoreToGet) {
                gameScene.nextScoreToGet += 30
                var blockTimer = Qt.createQmlObject(
                            "import QtQuick 2.3; Timer {}", game)
                ...
                blockTimer.triggered.connect(function () {
                    ...
                    for (var i = 0; i < gameScene.balls.length; i++) {
                        ...
                        gameScene.balls[i].linearVelocity = Qt.point(
                                    currentX < 0 ? currentX - 3 : currentX + 3,
                                                   currentY < 3 ? currentY - 3 : currentY + 3)
                    }
                })
                blockTimer.start()
            }
        ...
        function reactToSpecialFeature(specialFeatureIndex) {
            ...
            case 2:
                ...
                aBall.x = Math.floor(
                            Math.random(
                                ) * ((gameScene.width - aBall.width) - aBall.width)) + aBall.width
            ...
            case 3:
                ...
                extraBall.x = Math.floor(
                            Math.random(
                                ) * ((gameScene.width - extraBall.width)
                                     - extraBall.width)) + extraBall.width
                extraBall.y = gameScene.height / 2 - (extraBall.height / 2)
                var extraBall2 = ball.createObject(gameScene)
                extraBall2.x = Math.floor(
                            Math.random(
                                ) * ((gameScene.width - extraBall2.width)
                                     - extraBall2.width)) + extraBall2.width
        ...
        MouseArea {
            onClicked: {
                if (!gameScene.gameBegun) {
                    ...
                    newBall.x = Math.floor(
                                Math.random(
                                    ) * ((gameScene.width - newBall.width)
